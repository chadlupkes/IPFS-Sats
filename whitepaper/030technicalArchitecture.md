...
A complete interaction — from a user requesting content to a creator receiving compensation — touches every layer. The user's application resolves the root CID of the requested content and queries the Host Discovery Layer, which returns a map of which hosts hold which blocks. The application constructs a delivery plan and initiates parallel AtomicSats exchanges, one per block. Each exchange is atomic: the block is released only when the Lightning payment is confirmed. Assembled blocks are verified against the Merkle DAG, and the complete content is delivered to the user.
