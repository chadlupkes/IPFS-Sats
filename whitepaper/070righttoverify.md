...
**The Lightning payment proves current availability.** When a client initiates a SatSwap retrieval, the atomic payment structure ensures the client pays only if a host delivers a block that matches the requested CID. If the data is corrupted, altered, or missing, the payment fails to settle. This proves that a host currently holds content matching the CID — a guarantee of availability and integrity at retrieval time, not at original publication time.
