...
| **Host** | A network participant that stores content blocks and serves them via AtomicSats exchanges, earning sats per block delivered. Hosts publish Host Registry Records to the Records Database to advertise their inventory, pricing, and performance metrics. |
| **Host Registry Record** | A record in the Records Database published by an AtomicSats host to advertise its block inventory, pricing, Lightning endpoint, and performance metrics. The Host Discovery Layer's query function operates against this table. Schema defined in Section 10.7. |
| **Hashed Timelock Contract (HTLC)** | A cryptographic construct used by the Lightning Network to enable atomic exchanges. An HTLC locks a payment until the recipient reveals a cryptographic preimage, which the sender verifies before the payment settles. AtomicSats uses HTLCs to enforce the atomicity of every block-for-sats exchange — no delivery, no payment; no payment, no delivery. |
...
| **AtomicSats** | The atomic block-for-sats exchange primitive of the IPFS-Sats protocol. A four-message handshake — WANT, QUOTE, PAYMENT_HASH, BLOCK — between any two nodes. Analogous to BitSwap but replacing reciprocal block exchange with direct Lightning micropayment. The atomicity of every exchange is enforced by the Lightning Network's HTLC mechanism. |

### Units and Acronyms

| Term | Definition |
|---|---|
| **sats (satoshis)** | The base unit of Bitcoin. 1 sat = 0.00000001 BTC. All AtomicSats prices, LYW balances, and protocol payments are denominated in sats or millisatoshis (msats). Sats are not a protocol token — they are Bitcoin. |
| **msats (millisatoshis)** | One thousandth of a satoshi. The unit used for AtomicSats pricing in the exchange protocol, enabling fine-grained per-block pricing below one satoshi. |
...
The standard wire format for Lightning Network payment requests used in AtomicSats QUOTE messages.
...
**What is AtomicSats and how is it different from BitSwap?**
BitSwap is the block exchange protocol used by IPFS. It operates on a reciprocity model — nodes exchange blocks with each other, and nodes that don't share get deprioritized. AtomicSats replaces reciprocity with direct payment: a requesting node pays sats for every block it receives, and a delivering node earns sats for every block it delivers. This eliminates free-riding structurally — there is no incentive to consume without contributing because consumption requires payment, and contribution is directly rewarded.
