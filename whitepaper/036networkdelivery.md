...
AtomicSats does not prescribe how many hosts serve a given piece of content. It prescribes how a client pays a host for a block. The number of hosts involved in any given delivery — and therefore the complexity of the payment flow — is determined entirely by what the Host Discovery Layer returns.
...
3. The client initiates an AtomicSats exchange with that host
...
In this case, the discovery layer returns a map: which hosts hold which blocks. The client — or a coordinating AtomicSats node acting on the client's behalf — uses this map to route block requests to the appropriate hosts simultaneously.
...
The spread mechanism means that participation in AtomicSats delivery is not limited to storage operators. Any Lightning node with sufficient connectivity and the AtomicSats protocol stack is a potential participant in the delivery economy.

---

## 3.6.4 Caching as a Capital Decision

When an AtomicSats node observes that a particular CID is generating high query volume through the discovery layer, caching those blocks locally converts a routing margin into a full delivery payment.
...
A block that fails verification triggers a failed payment for that block and a retry from an alternate host if one is available in the discovery results. The atomic nature of the AtomicSats exchange ensures that a host cannot receive payment for a block the client cannot verify.
