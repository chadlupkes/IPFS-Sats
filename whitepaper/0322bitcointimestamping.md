This section is highly detailed and technically sound. To clean it up for a formal white paper, we should separate the conceptual design from the implementation specifics and use consistent, clear formatting.

Here is a revised structure that focuses on clarity and hierarchy, followed by a suggestion for the implementation code.
...
This application-level chain is built by modifying the **Metadata Bundle** structure to include a cryptographic link to its predecessor. Each new content's metadata bundle contains the **complete hash of its parent's metadata bundle** (which itself includes the parent's content CID).
  ...
  * **Original Content ($M_1$):** $\text{parent\_bundle\_hash} = \text{null}$. Anchored on-chain.
  * **Derivative Work ($M_2$):** $\text{parent\_bundle\_hash} = \text{Hash}(M_1)$.
  * **Subsequent Work ($M_3$):** $\text{parent\_bundle\_hash} = \text{Hash}(M_2)$.

When $M_3$ is eventually anchored in a new Bitcoin transaction, the entire chain ($M_1 \to M_2 \to M_3$) is timestamped simultaneously.
